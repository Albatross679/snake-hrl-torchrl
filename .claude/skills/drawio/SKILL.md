...
On headless Linux (no display server), prefix with `xvfb-run` and add `--no-sandbox`:

```bash
xvfb-run drawio --no-sandbox -x -f <format> -e -b 10 -o <output> <input.drawio>
```

On systems with a display server, the simpler form works:
...
**Rounded rectangle:**
```xml
<mxCell id="2" value="Label" style="html=1;rounded=1;whiteSpace=wrap;" vertex="1" parent="1">
...
**Diamond (decision):**
```xml
<mxCell id="3" value="Condition?" style="html=1;rhombus;whiteSpace=wrap;" vertex="1" parent="1">
...
## CRITICAL: HTML rendering in VS Code

**Always include `html=1;` in the `style` attribute of any `mxCell` whose `value` contains HTML markup** (e.g., `&lt;b&gt;`, `&lt;br&gt;`, `&lt;i&gt;`, `&lt;font&gt;`). Without `html=1`, the VS Code Draw.io extension renders raw HTML tags as literal text instead of formatting them.

```xml
<!-- CORRECT: html=1 enables HTML rendering -->
<mxCell id="2" value="&lt;b&gt;Title&lt;/b&gt;&lt;br&gt;subtitle" style="html=1;rounded=1;whiteSpace=wrap;" vertex="1" parent="1">

<!-- WRONG: missing html=1, tags show as raw text -->
<mxCell id="2" value="&lt;b&gt;Title&lt;/b&gt;&lt;br&gt;subtitle" style="rounded=1;whiteSpace=wrap;" vertex="1" parent="1">
```

**Rule of thumb:** If the `value` attribute contains any `&lt;` entity, the style must have `html=1;`.
