...
Field specifications for SL Tree (Level 1b), SL Tree Regression (Level 2b), and project task-specific configs (Level 3). All inherit Base fields and built-in infrastructure (output, console, checkpointing, metricslog, experiment tracking).
...
Tree models log metrics reflecting boosting-round-based training. All metrics are dual-logged to both `metrics.jsonl` (local) and the experiment tracker (W&B via `log_epoch_metrics`):
