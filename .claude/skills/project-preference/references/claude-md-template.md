...
- GitHub: `{GITHUB_USERNAME}`, email `{EMAIL}`
...
- Project ID: `{TODOIST_PROJECT_ID}`

## LaTeX Report (Optional)

If the project includes a LaTeX report, add this section. Compiled with **LaTeX Workshop** (VSCode extension).

{Tree of the report directory. Example:}

\`\`\`
report/
├── report.tex          # main compilation target
├── header.tex          # shared package imports and macros
├── some-style.sty      # conference/journal style file
└── report.pdf          # compiled output
\`\`\`

**Build rules:**
- LaTeX Workshop handles compilation automatically on save — do NOT run `pdflatex`/`latexmk` manually from the terminal.
- Images and plots should go in `media/` and be included with `\includegraphics`.
- Do not modify provided style files (`.sty`).
...
## GPU Task Safety (Optional — for GPU projects)
