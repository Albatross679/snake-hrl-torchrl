...
- [x] **Phase 02.1: Re-collect with per-node phase encoding** - Re-collect transitions with per-node CPG phase as explicit state feature (INSERTED)
- [x] **Phase 02.2: Collect RL-step-only transitions** - Collect only RL-level steps, no sub-steps (INSERTED)
