...
class FlatStepDataset(Dataset):
    ...
    Loads batch files produced by collect_data.py with config.flat_output=True.
    Each file contains flat (state, action, next_state) transitions — one row
    per RL step, unlike the Phase 02.1 checkpoint format (substep_states).
    ...
    def __init__(
        ...
        Args:
            data_dir: Directory containing batch_*.pt files (Phase 02.2 flat format).
            split: "train" or "val".
            val_fraction: Fraction of episodes held out for validation.
    ...
    def get_sample_weights(self, n_bins: int = 20, clip_max: float = 10.0) -> torch.Tensor:
        ...
        Useful for WeightedRandomSampler to upsample rare states.

        Args:
            n_bins: Number of bins per feature dimension (uses first n_bins features).
            clip_max: Maximum weight value (prevents extreme outlier dominance).
