#!/usr/bin/env python
"""Record video of snake locomotion rollout (PyElastica backend).
...
Usage:
    ...
    python -m locomotion_elastica.record --steps 200 \
        --output media/locomotion_elastica_passive.mp4

    # With trained policy
    python -m locomotion_elastica.record \
        --checkpoint model/locomotion_elastica/best.pt \
        --gait forward \
        --output media/locomotion_elastica_forward.mp4

    # GIF output
    python -m locomotion_elastica.record \
        --checkpoint model/locomotion_elastica/best.pt \
        --gait turn_left \
        --output media/locomotion_elastica_turn.gif
"""
...
from locomotion_elastica.config import (
    ...
    LocomotionElasticaNetworkConfig,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Record snake locomotion rollout (PyElastica)"
    )
    parser.add_argument(
        "--gait",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="media/locomotion_elastica.mp4",
...
def load_actor(checkpoint_path: str, env: LocomotionElasticaEnv, device: str):
...
def collect_rollout(env, actor, max_steps, device):
    ...
    for step in range(max_steps):
        ...
        td = env.step(td)
        rewards_list.append(td["next", "reward"].item())
...
def build_animation(rollouts, gait, elevation, azimuth, fps, snake_radius):
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(f"Snake Locomotion (PyElastica) — {gait.value}")
...
def main():
    ...
    config = LocomotionElasticaConfig(seed=args.seed, device=args.device)
    config.env.gait = GaitType(args.gait)
    config.env.device = args.device

    env = LocomotionElasticaEnv(config.env, device=args.device)
    ...
    print(
        ...
        f"up to {max_steps} steps each (gait={args.gait}, backend=PyElastica)..."
    )
    rollouts = []
    total_reward = 0.0
    total_steps = 0
    ...
    fig, ani = build_animation(
        ...
        fps=args.fps,
        snake_radius=config.env.physics.geometry.snake_radius,
    )
