...
def build_mjcf_xml(physics: Zheng2022PhysicsConfig) -> str:
    ...
    Args:
        physics: Snake physics parameters.

    Returns:
        MJCF XML string.
    """
    link_len = physics.link_length
    radius = physics.link_radius
    stiffness = physics.joint_stiffness
    damping = physics.joint_damping
    armature = physics.armature
    joint_range = physics.joint_range_deg
    gear = physics.motor_gear
    force_range = physics.motor_force_range
    ...
    <body name="link_0" pos="0 0 0">
      ...
      <geom name="g_0" type="capsule" fromto="0 0 0 {link_len} 0 0"
            size="{radius}" density="{physics.link_density}"/>
      <site name="head_site" pos="0 0 0" size="0.001"/>
"""

    # Links 1 through N-1 (nested)
    for i in range(1, physics.num_links):
...
{indent * i}        size="{radius}" density="{physics.link_density}"/>
"""
  ...
  <option timestep="{physics.sim_timestep}"
          gravity="0 0 0"
          density="{physics.fluid_density}"
          viscosity="{physics.fluid_viscosity}">
    <flag energy="enable"/>
  </option>
...
class UnderwaterSnakeEnv(EnvBase):
    ...
    def __init__(
        self,
        config: Optional[Zheng2022EnvConfig] = None,
        device: str = "cpu",
        batch_size: Optional[torch.Size] = None,
    ):
        self.config = config or Zheng2022EnvConfig()
        self._physics = self.config.physics
        self._device = device

        super().__init__(device=device, batch_size=batch_size or torch.Size([]))

        # Build and load MuJoCo model
        xml_string = build_mjcf_xml(self._physics)
        self.model = mujoco.MjModel.from_xml_string(xml_string)
        self.data = mujoco.MjData(self.model)

        # Cache joint/actuator indices
        self._num_joints = self._physics.num_joints  # 6
    ...
    def _make_spec(self) -> None:
        ...
        self.action_spec = BoundedTensorSpec(
            low=-1.0,
            high=1.0,
            shape=(self._physics.action_dim,),
            dtype=torch.float32,
            device=self._device,
        )
    ...
    def _step(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        for _ in range(self._physics.sim_steps_per_control):
