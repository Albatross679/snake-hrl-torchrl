#!/usr/bin/env python
"""Train the coil skill using PPO."""

import argparse
import torch
from pathlib import Path

from snake_hrl.envs import CoilEnv
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train coil skill")
    parser.add_argument(
        "--total-frames",
        type=int,
        default=500_000,
...
def main():
    ...
    # Create environment
    # Coil env starts snake close to prey
    env_config = CoilEnvConfig(
        max_episode_steps=500,
        use_reward_shaping=True,
        min_coil_wraps=1.5,
        contact_fraction_threshold=0.6,
    )
    env = CoilEnv(config=env_config, device=args.device)
