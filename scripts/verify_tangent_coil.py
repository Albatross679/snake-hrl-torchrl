#!/usr/bin/env python3
"""Verify coiling starting from tangent position to prey.

The key insight: curvature control alone doesn't cause locomotion.
We need to position the snake so it's already touching/tangent to prey,
then curvature will wrap it around.
"""
...
def create_tangent_positions(
    num_nodes: int,
    prey_center: np.ndarray,
    prey_radius: float,
    snake_length: float,
    tangent_angle: float = 0.0,  # Where snake touches prey (radians)
    offset: float = 0.005,  # Small gap
) -> np.ndarray:
    """
    Create snake positions tangent to prey cylinder.

    The snake head starts at the tangent point and extends straight
    away from the prey surface.
    """
    # Tangent point on prey surface
    contact_radius = prey_radius + offset
    contact_point = prey_center[:2] + contact_radius * np.array([
        np.cos(tangent_angle), np.sin(tangent_angle)
    ])

    # Tangent direction (perpendicular to radial)
    # For counterclockwise wrapping, tangent points in +theta direction
    tangent_dir = np.array([-np.sin(tangent_angle), np.cos(tangent_angle)])

    # Create straight snake along tangent
    segment_length = snake_length / (num_nodes - 1)
    positions = np.zeros((num_nodes, 3))

    for i in range(num_nodes):
        pos_2d = contact_point + i * segment_length * tangent_dir
...
def run_tangent_coil_experiment(
    tangent_angle: float,
    curvature_value: float,
    max_steps: int = 300,
    verbose: bool = True,
) -> dict:
    """Run coiling experiment from tangent position."""
    ...
    # Create tangent initial positions
    tangent_positions = create_tangent_positions(
        num_nodes=num_nodes,
    ...
    # Inject positions
    inject_positions(robot, tangent_positions)
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Tangent angle: {np.degrees(tangent_angle):.1f}°, Curvature: {curvature_value:.2f}")
        print(f"Initial contact: {initial_state['contact_fraction']:.3f}")
        print(f"{'='*60}")

    # Tracking
    history = {
        'positions': [tangent_positions.copy()],
...
def create_visualization(result: dict, output_dir: Path):
    ...
    axes[1, 3].text(0.5, 0.5,
                   f"Tangent angle: {np.degrees(result['tangent_angle']):.1f}°\n"
    ...
    fig.suptitle(f"Tangent to Coil - {success_str}", fontsize=14)
    plt.tight_layout()

    filename = f"tangent_coil_angle{int(np.degrees(result['tangent_angle']))}_k{result['curvature']:.1f}.png"
...
def main():
    print("="*60)
    print("VERIFICATION: Tangent Position to Coil")
    print("="*60)
    print("\nStrategy: Start snake TANGENT to prey, apply curvature to wrap")
    print()

    output_dir = Path("Media/tangent_coil")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Test different tangent angles and curvatures
    angles = [0, np.pi/4, np.pi/2]  # 0°, 45°, 90°
    curvatures = [8.0, 9.0, 10.0]
    ...
    for angle in angles:
        for curv in curvatures:
            result = run_tangent_coil_experiment(
                tangent_angle=angle,
            ...
            if result['success'] and successful_result is None:
                ...
                # Save trajectory
                np.savez(
                    output_dir / "successful_tangent_coil.npz",
                    **result['history'],
                    tangent_angle=result['tangent_angle'],
                    curvature=result['curvature'],
                )

                # Create video
                create_video(result, output_dir / "successful_tangent_coil.mp4")

    # Summary
    print("\n" + "="*60)
    print("RESULTS SUMMARY")
    print("="*60)
    print(f"\n{'Angle':>8} {'κ':>6} {'Success':>8} {'MaxContact':>10} {'MaxWrap':>10}")
    print("-"*50)

    for r in results:
        angle_deg = np.degrees(r['tangent_angle'])
        success_str = "YES" if r['success'] else "no"
        print(f"{angle_deg:>8.1f} {r['curvature']:>6.1f} {success_str:>8} "
              f"{r['max_contact']:>10.3f} {r['max_wrap']:>10.3f}")

    if successful_result:
        print(f"\nSUCCESS! Trajectory saved.")
    else:
        print("\nNo success. Looking at best result:")
        best = max(results, key=lambda r: r['max_contact'] + r['max_wrap'])
        print(f"  Angle: {np.degrees(best['tangent_angle']):.1f}°")
