...
from zheng2022.env_zheng2022 import UnderwaterSnakeEnv, build_mjcf_xml
...
class TestMJCFModel:
    ...
    def test_xml_generation(self):
        physics = Zheng2022PhysicsConfig()
        xml = build_mjcf_xml(physics)
        assert isinstance(xml, str)
        assert "underwater_snake_zheng2022" in xml
        assert f'density="{physics.fluid_density}"' in xml
        assert f'viscosity="{physics.fluid_viscosity}"' in xml
    ...
    def test_model_structure(self):
        ...
        assert model.nbody == physics.num_links + 1

        # 6 hinge joints + 1 freejoint (7 total in joint array)
        # Freejoint expands to 7 qpos (3 pos + 4 quat) and 6 qvel
        assert model.njnt == physics.num_joints + 1  # 6 hinge + 1 free

        # 6 motor actuators
        assert model.nu == physics.num_joints

    def test_custom_stiffness(self):
        physics = Zheng2022PhysicsConfig(joint_stiffness=2.5)
        xml = build_mjcf_xml(physics)
...
class TestCurriculumReward:
    ...
    @pytest.fixture
    def curriculum(self):
        return CurriculumReward(Zheng2022CurriculumConfig())
